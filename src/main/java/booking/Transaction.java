  ...
  private void showFreePlaces() {
    ...
    switch (typePlace) {
      ...
      case 1:
        ...
        cost = movie.getPriceNormal();
        freePlace = places.stream().filter(place -> place.getTypeOfPlaces() == TypeOfPlaces.NORMAL)
            .collect(
                Collectors.toList());
        int rowNumb = 0;
        String rows = "";
        for (Place place : freePlace) {
          if (!place.isBusy()) {
            if (rowNumb != place.getRowNumber()) {
              rows = rows + "\n";
              rows = rows + "Row - " + place.getRowNumber() + ", place number - " + place
                  .getPlaceNumber();
              rowNumb = place.getRowNumber();
            } else {
              rows = rows + ", " + place.getPlaceNumber();
            }
          }
        }
      ...
      case 2:
        ...
        cost = movie.getPricePremium();
        freePlace = places.stream().filter(place -> place.getTypeOfPlaces() == TypeOfPlaces.PREMIUM)
            .collect(
                Collectors.toList());
        int rowNumb1 = 0;
        String rows1 = "";
        for (Place place : freePlace) {
          if (!place.isBusy()) {
            if (rowNumb1 != place.getRowNumber()) {
              rows1 = rows1 + "\n";
              rows1 = rows1 + "Row - " + place.getRowNumber() + ", place number - " + place
                  .getPlaceNumber();
              rowNumb1 = place.getRowNumber();
            } else {
              rows1 = rows1 + ", " + place.getPlaceNumber();
            }
          }
        }
